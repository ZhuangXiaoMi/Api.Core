...
namespace Api.Core.AOP
{
    public abstract class BaseCacheAOP : IInterceptor
    {
        ...
        protected string CustomCacheKey(IInvocation invocation)
        {
            ...
            var methodArguments = invocation.Arguments.Select(GetArgumentValue).Take(3).ToList();//获取前三个参数
        ...
        private static string Resolve(Expression expression)
        {
            ...
            if (expression is BinaryExpression)
            {
                ...
                if (binary.Left is MethodCallExpression && binary.Right is ConstantExpression)//解析x=>x.Name.Contains("xxx")==false这类
                {
                ...
                if ((binary.Left is MemberExpression && binary.Right is MemberExpression)
                    || (binary.Left is MemberExpression && binary.Right is UnaryExpression))//解析x=>x.Date==DateTime.Now这类
                {
            ...
            if (expression is UnaryExpression)
            {
                ...
                if (unary.Operand is MemberExpression && unary.NodeType == ExpressionType.Not)//解析x=>!x.isDeletion这类
                {
                    ConstantExpression constant = Expression.Constant(false);
                    return ResolveFunc(unary.Operand, constant, ExpressionType.Equal);
                }
            }
            if (expression is MemberExpression && expression.NodeType == ExpressionType.MemberAccess)//解析x=>x.isDeletion这类
            {
                MemberExpression member = expression as MemberExpression;
                ConstantExpression constant = Expression.Constant(true);
                return ResolveFunc(member, constant, ExpressionType.Equal);
            }
            if (expression is MethodCallExpression)//解析x=>x.Name.Contains("xxx")或array.Contains(x.Name)这类
            {
        ...
        private static string ResolveFunc(Expression left, Expression right, ExpressionType expressiontype)
        {
            ...
            var oper = GetOperator(expressiontype);
            return name + oper + value ?? "null";
        }
        ...
        private static string In(MethodCallExpression expression, object isTrue)
        {
            ...
            string result = string.Format("{0} {1} ({2})", name, oper, compName);
            return result;
        }
        ...
        private static string Len(MethodCallExpression expression, object value, ExpressionType expressiontype)
        {
            ...
            string oper = GetOperator(expressiontype);
            string result = string.Format("len({0}){1}{2}", name, oper, value.ToString());
            return result;
        }
    }
}